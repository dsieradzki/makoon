...
use crate::operator::repository::{Error, Result};
...
impl JsonRepository {
        ...
        match repo.load() {
            ...
            Err(e) => {
                match e.clone() {
                    ...
                    Error::IO(e) => {
                        warn!("Cannot load database file, error: [{}], create new database file", e);
                        repo.save(
                            DbData {
                                clusters: vec![],
                                action_log: vec![],
    ...
    fn save(&self, data: DbData) -> Result<()> {
        ...
        let result = file.write_all(result.as_bytes()).map_err(|e| Error::DB(e.to_string()));
        drop(mutex);
        return result;
    }
