...
impl Client {
    ...
    pub fn connect(
        ...
        let tcp = TcpStream::connect(format!("{}:{}", ip_address, self.port))
            .map_err(|e| e.to_string())?;
        let mut session = ssh2::Session::new().map_err(|e| e.to_string())?;
        session.set_tcp_stream(tcp);
        session.handshake().map_err(|e| e.to_string())?;
        session
            .userauth_pubkey_memory(username, Some(public_key), private_key, None)
            .map_err(|e| e.to_string())?;
    ...
    pub fn upload_file(&self, file_path: &str, content: &str) -> Result<(), String> {
        ...
        let mut remote_file = session
            .scp_send(Path::new(file_path), 0o644, content_length, None)
            .map_err(|e| e.to_string())?;
