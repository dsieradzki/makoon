...
impl Operator {
                    ...
                    match rx.try_recv() {
                        ...
                        Err(_) => {
                            std::thread::sleep(Duration::from_millis(
                                config.worker_thread_probe_duration,
                            ));
    ...
    pub fn create_cluster(
        ...
        let cluster = Cluster {
            ...
            helm_apps: cluster_request
                ...
                .collect(),
            cluster_resources: cluster_request
                .cluster_resources
                .into_iter()
                .map(|mut i| {
                    i.id = uuid::Uuid::new_v4().to_string();
    ...
    pub fn add_node_cluster(
        &self,
        access: AccessData,
        cluster_name: String,
        node_request: ClusterNode,
    ) -> crate::Result<ClusterNode> {
        ...
        self.repository.save_log(LogEntry::info(
            &cluster_name,
            format!(
                "Adding node [{}] to cluster [{}] has been started",
                node_request.name, cluster_name
            ),
        ))?;
    ...
    pub fn delete_cluster(&self, access: AccessData, cluster_name: String) -> crate::Result<()> {
        ...
        self.repository.save_log(LogEntry::info(
            &cluster_name,
            "Cluster deletion started".to_string(),
        ))?;
    ...
    pub fn delete_node_from_cluster(
        &self,
        access: AccessData,
        cluster_name: String,
        node_name: String,
        ...
        self.repository.save_log(LogEntry::info(
            &cluster_name,
            format!(
                "Deleting node [{}] from cluster has been started",
                node_name
            ),
        ))?;
    ...
    pub fn get_clusters(&self) -> crate::Result<Vec<ClusterHeader>> {
        info!("Get clusters");
        let repo = self.repository.clone();

        let result = repo.get_clusters().map(|r| {
            r.into_iter()
                .map(|i| -> ClusterHeader {
                    let cores = i.nodes.iter().map(|i| i.cores).reduce(|a, b| a + b);
    ...
    pub fn cluster_status(&self, cluster_name: &str) -> crate::Result<Vec<ClusterNodeStatus>> {
        ...
        let mut result: Vec<ClusterNodeStatus> = cluster
            .nodes
            .iter()
    ...
    pub fn apps_status(&self, cluster_name: &str) -> crate::Result<Vec<AppStatus>> {
        ...
        let installed_releases =
            ssh_client.execute_to(&helm_client::new(HELM_CMD).sudo().list().all().json().build());
    ...
    pub fn logs_for_cluster(&self, name: &str) -> crate::Result<Vec<LogEntry>> {
        info!("Get logs for the cluster");
        Ok(self.repository.logs(name)?)
    }

    pub fn clear_logs_for_cluster(&self, name: &str) -> crate::Result<()> {
        info!("Clear logs for the cluster");
        self.repository.delete_logs(name)?;
        Ok(())
    }
    pub fn get_cluster(&self, cluster_name: &str) -> crate::Result<Option<Cluster>> {
        info!("Get cluster");
        Ok(self.repository.get_cluster(cluster_name)?)
    }

    pub fn get_nodes(&self, cluster_name: &str) -> crate::Result<Vec<ClusterNode>> {
    ...
    pub fn save_helm_app(&self, cluster_name: &str, app: HelmApp) -> crate::Result<String> {
    ...
    pub fn update_helm_app(&self, cluster_name: &str, app: HelmApp) -> crate::Result<()> {
    ...
    pub fn delete_helm_app(&self, cluster_name: &str, app_id: &str) -> crate::Result<()> {
    ...
    pub fn install_helm_app(&self, cluster_name: &str, app_id: &str) -> crate::Result<()> {
        ...
        crate::dispatcher::install_helm_app(&ssh_client, app)?;
        Ok(())
    }
    pub fn uninstall_helm_app(&self, cluster_name: &str, app_id: &str) -> crate::Result<()> {
    ...
    pub fn update_cluster_workload(&self, cluster_name: &str, res: ClusterResource) -> crate::Result<()> {
    ...
    pub fn install_cluster_workload(&self, cluster_name: &str, res_id: &str) -> crate::Result<()> {
    ...
    pub fn uninstall_cluster_workload(&self, cluster_name: &str, res_id: &str) -> crate::Result<()> {
        ...
        ssh_client
            .execute(format!("sudo microk8s.kubectl delete -f /tmp/{}.yaml", file_name).as_str())?;
        ssh_client.execute(format!("sudo rm /tmp/{}.yaml", file_name).as_str())?;
        Ok(())
    }

    pub fn delete_cluster_workload(&self, cluster_name: &str, res_id: &str) -> crate::Result<()> {
