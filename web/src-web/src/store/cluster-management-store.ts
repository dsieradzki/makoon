...
class ClusterManagementStore {
    ...
    get cpuSum() {
        return (this.cluster.nodes ?? []).reduce((acc, i) => acc + i.cores, 0)
    }

    get ramSum() {
        return (this.cluster.nodes ?? []).reduce((acc, i) => acc + i.memory, 0)
    }

    get disksSizeSum() {
        return (this.cluster.nodes ?? []).length * this.cluster.diskSize;
    }

    async loadProject(clusterName: string) {
        const cluster = await apiCall(() => api.clusters.getCluster(clusterName), LOADING_CLUSTER);
    ...
    async loadLogs(clusterName: string) {
        const logs = await apiCall(() => api.clusters.logsForCluster(clusterName), LOADING_LOGS);
    ...
    async clearLogs() {
        await api.clusters.clearLogsForCluster(this.cluster.clusterName);
        runInAction(() => {
            this.logs = [];
        });
    }
