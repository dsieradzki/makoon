...
const HelmAppStatus = (props: HelmAppStatusComponentProps) => {
    switch (props.status) {
        ...
        case AppStatusType.Deployed:
            return <Tag className={`${props.className} h-[23px] opacity-80`}
        ...
        case AppStatusType.NotInstalled:
            return <Tag className={`${props.className} h-[23px]`}
                        severity={"info"} value="Not installed"/>
        case null:
            return <ProgressSpinner style={{width: '15px', height: '15px'}} strokeWidth="10"/>
        default:
            return <Tag className={`${props.className} h-[23px]`}
