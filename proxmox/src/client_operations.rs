...
impl ClientOperations {
    ...
    pub fn create_virtual_machine(&self, req: CreateVirtualMachine) -> Result<String> {
        ...
        Ok(
            self.http.post::<CreateVirtualMachine, Data<String>>(
                &self.token,
    ...
    pub fn resize_disk(&self, req: ResizeDisk) -> Result<Option<String>> {
        debug!("Resize disk");
        Ok(
            self.http.put::<ResizeDisk, Data<Option<String>>>(&self.token,
    ...
    pub fn permissions(&self) -> Result<HashMap<String, HashMap<String, i64>>> {
        ...
        Ok(self.http.get::<Data<HashMap<String, HashMap<String, i64>>>>(&self.token, "/access/permissions")?.data)
    }
    ...
    pub fn storage(&self, node: String, storage_content_type: Option<StorageContentType>) -> Result<Vec<Storage>> {
        debug!("Get storage");
        ...
        Ok(self.http.get::<Data<Vec<Storage>>>(&self.token, url.as_str())?.data)
    }
    ...
    pub fn nodes(&self) -> Result<Vec<Node>> {
        ...
        Ok(self.http.get::<Data<Vec<Node>>>(&self.token, "/nodes")?.data)
    }

    #[doc = "List available networks."]
    #[doc = "Required permissions: Accessible by all authenticated users."]
    pub fn networks(&self, node: String, network_type: Option<NetworkType>) -> Result<Vec<Network>> {
        debug!("Get networks");
        ...
        Ok(self.http.get::<Data<Vec<Network>>>(&self.token, url.as_str())?.data)
    }

    #[doc = "Virtual machine index (per node)."]
    #[doc = "Required permissions: Only list VMs where you have VM.Audit permissons on /vms/<vmid>."]
    pub fn virtual_machines(&self, node: String, full: Option<bool>) -> Result<Vec<VirtualMachine>> {
        ...
        Ok(self.http.get::<Data<Vec<VirtualMachine>>>(&self.token, url.as_str())?.data)
    }

    #[doc = "LXC container index (per node)."]
    #[doc = "Required permissions: Only list CTs where you have VM.Audit permissons on /vms/<vmid>."]
    pub fn lxc_containers(&self, node: String) -> Result<Vec<LxcContainer>> {
        debug!("Get LCX containers");
        Ok(self.http.get::<Data<Vec<LxcContainer>>>(&self.token, format!("/nodes/{}/lxc", node).as_str())?.data)
    }

    #[doc = "Start virtual machine."]
    /// Check: ["#perm","/vms/{vmid}",["VM.PowerMgmt"]]
    pub fn start_vm(&self, node: String, vm_id: u32) -> Result<String> {
        debug!("Start VM [{}]", vm_id);
        Ok(
            self.http.post::<(), Data<String>>(
                &self.token,
    ...
    /// Check: ["#perm","/vms/{vmid}",["VM.PowerMgmt"]]
    pub fn shutdown_vm(&self, node: String, vm_id: u32) -> Result<String> {
    ...
    /// Check: ["#perm","/vms/{vmid}",["VM.PowerMgmt"]]
    pub fn stop_vm(&self, node: String, vm_id: u32) -> Result<String> {
        debug!("Stop VM [{}]", vm_id);
        Ok(
            self.http.post::<(), Data<String>>(
                &self.token,
    ...
    pub fn delete_vm(&self, node: String, vm_id: u32) -> Result<String> {
        debug!("Delete VM [{}]", vm_id);
        Ok(
            self.http.delete::<Data<String>>(
                &self.token,
    ...
    pub fn status_vm(&self, node: String, vm_id: u32) -> Result<VmCurrentStatus> {
        Ok(
            self.http.get::<Data<VmCurrentStatus>>(
                &self.token,
    ...
    pub fn download_image(&self, req: DownloadImage) -> Result<String> {
        Ok(
            self.http.post::<DownloadImage, Data<String>>(
                &self.token,
    ...
    pub fn storage_content(&self, node: String, storage: String) -> Result<Vec<StorageContent>> {
        Ok(
            self.http.get::<Data<Vec<StorageContent>>>(
                &self.token,
    ...
    pub fn storage_content_details(&self, node: String, storage: String, volid: String) -> Result<StorageContentDetails> {
        Ok(
            self.http.get::<Data<StorageContentDetails>>(
                &self.token,
